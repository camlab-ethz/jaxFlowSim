...
plotting_dir=$(pwd)
openBF_timing_script=run_timing.jl
jaxFlowSim_timing_script=run_timing.py
...
python run_timing.py $samples $num_vessels_file $network_names | grep "elapsed time =" | awk '{print $4}' > "$plotting_dir/$jaxFlowSim_timing_file"
rm $jaxFlowSim_timing_script
cp $num_vessels_file $plotting_dir
rm $num_vessels_file
cd $plotting_dir

python plot_timing.py $openBF_timing_file $jaxFlowSim_timing_file $num_vessels_file $samples
