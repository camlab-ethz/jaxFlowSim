...
from src.anastomosis import solveAnastomosis, solveAnastomosis_nn
from src.conjunctions import solveConjunction, solveConjunction_nn
from src.bifurcations import solveBifurcation, solveBifurcation_nn
from src.boundary_conditions import setInletBC, setOutletBC, setOutletBC_nn
from src.utils import pressureSA, pressureSA_nn, waveSpeedSA
...
def solveModel_nn(N, B, 
    ...
    sim_dat = sim_dat.at[:,B:-B].set(muscl_nn(dt, 
                  sim_dat[1,B:-B],
                  sim_dat[2,B:-B], 
                  sim_dat_const[0,B:-B], 
                  sim_dat_const[1,B:-B], 
    ...
    def bodyFun(j, dat):
        (sim_dat, sim_dat_aux, sim_dat_const, sim_dat_const_aux, edges, rho, strides, nn_params) = dat
        end = strides[j,1]
        ...
        def setOutletBCWrapper(sim_dat, sim_dat_aux):
            ...
            W1M0 = sim_dat_aux[j,0]
            W2M0 = sim_dat_aux[j,1]
            u, Q, A, c, P1, Pc = setOutletBC_nn(dt,
                                             u1, u2, Q1, A1, c1, c2, 
                                             P1, P2, P3, Pc, W1M0, W2M0,
                                             sim_dat_const[0,end-1],
                                             sim_dat_const[1,end-1],
        ...
        def solveBifurcationWrapper(sim_dat):
            ...
            (u1, u2, u3, 
             ...
             A1, A2, A3, 
             c1, c2, c3, 
             P1, P2, P3) = solveBifurcation_nn(u1, u2, u3, 
                                            A1, A2, A3,
                                            sim_dat_const[0,end-1],
                                            sim_dat_const[0,d1_i_start],
                                            sim_dat_const[0,d2_i_start],
        ...
        def solveConjunctionWrapper(sim_dat, rho):
            ...
            A2 = sim_dat[2,d_i_start]
            (u1, u2, Q1, Q2, 
             A1, A2, c1, c2, P1, P2) = solveConjunction_nn(u1, u2, 
                                                        A1, A2,
                                                        sim_dat_const[0,end-1],
                                                        sim_dat_const[0,d_i_start],
                                                        sim_dat_const[1,end-1],
        ...
        def solveAnastomosisWrapper(sim_dat):
            ...
            u1, u2, u3, Q1, Q2, Q3, A1, A2, A3, c1, c2, c3, P1, P2, P3 = lax.cond(
                jnp.maximum(p1_i, p2_i) == j, 
                lambda: solveAnastomosis_nn(u1, u2, u3, 
                                         A1, A2, A3,
                                         sim_dat_const[0,end-1],
                                         sim_dat_const[0,p1_i_end-1],
                                         sim_dat_const[0,d_start],
...
def muscl_nn(dt, 
    ...
    s_A = vmap(lambda a: jnp.sqrt(a))(A)
    Q = vmap(lambda a, b, c, d, e: a - dt*(viscT[0]*a/b + c*(d - e)*b))(Q, A, wallE, s_A, s_A0)
    P = vmap(lambda a, b, c, d: pressureSA_nn(a*b, c, d, nn_params))(s_A, s_inv_A0, beta, Pext)
    c = vmap(waveSpeedSA)(s_A, gamma)
    u = vmap(lambda a, b: a/b)(Q, A)
