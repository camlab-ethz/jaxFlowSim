...
from src.solver import computeDt, solveModel, solveModel_nn
...
def configSimulation(input_filename, verbose=False, make_results_folder=True):
    ...
    (sim_dat, sim_dat_aux, 
     ...
     masks, strides, edges, 
     vessel_names, input_data) = buildArterialNetwork(data["network"], blood)#, junction_functions) = buildArterialNetwork(data["network"], blood)
...
def simulationLoopUnsafe_nn(N, B,
                        sim_dat, sim_dat_aux, 
                        sim_dat_const, sim_dat_const_aux,
                        Ccfl, input_data, rho, 
                        masks, strides, edges,
                        nn_params, upper = 100000):
    t = 0.0
    dt = 1
    P_t = jnp.zeros((upper,5*N))
    t_t = jnp.zeros(upper)
    ...
    def bodyFun(i, args):
        (sim_dat, sim_dat_aux, sim_dat_const, 
         ...
         P_t, nn_params) = args
        dt = computeDt(Ccfl, sim_dat[0,:],sim_dat[3,:], sim_dat_const[-1,:])
        sim_dat, sim_dat_aux = solveModel_nn(N, B, 
                                          t, dt, input_data, rho,
                                          sim_dat, sim_dat_aux, 
                                          sim_dat_const, sim_dat_const_aux, 
                                          masks, strides[:,:2], edges, nn_params)
        t = (t + dt)%sim_dat_const_aux[0,0]
        t_t = t_t.at[i].set(t)
        P_t = P_t.at[i,:].set(saveTempData(N, strides, sim_dat[4,:]))
    ...
    (sim_dat, sim_dat_aux, sim_dat_const, 
     ...
     edges, 
     input_data, rho, 
     P_t, nn_params) = lax.fori_loop(0, upper, bodyFun, (sim_dat, sim_dat_aux, sim_dat_const, 
                                                sim_dat_const_aux,
                                                dt, t, t_t,
                                                edges, 
                                                input_data, rho,
...
def simulationLoop(N, B, num_snapshots, 
    ...
    def bodyFun(args):
        ...
        return (sim_dat, sim_dat_aux, sim_dat_const, sim_dat_const_aux, 
                t, counter, timepoints, passed_cycles, dt, 
                P_t, P_l, t_t, 
                conv_tol, Ccfl, edges, 
                input_data, rho)
