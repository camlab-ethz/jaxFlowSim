...
sim_dat_obs_long, t_obs_long, P_obs_long = sim_loop_wrapper(0.5, upper=120000)
    ...
    def relative_loss(self, s, s_pred):
        return jnp.log(
            jnp.mean(
                jnp.power(
                    jnp.linalg.norm(
                        s_pred[-1, [-1, -6]] - s[-1, [-1, -6]], ord=None, axis=self.axis
                    ),
                    2,
                )
                / jnp.power(
                    jnp.linalg.norm(s[-1, [-1, -6]], ord=None, axis=self.axis), 2
                )
            )
        )
...
def logp(y, r, sigma):
    ...
    """
    _, _, y_hat = sim_loop_wrapper(r)  # pylint: disable=E1102
    # log_prob = jnp.mean(
    #    jnp.linalg.norm(
...
for set_num, setup in enumerate(settings):
    ...
    _, t, y = sim_loop_wrapper(R)  # pylint: disable=E1102
    loss_val = loss(P_obs, y)
    _, t, y = sim_loop_wrapper(R, upper=120000)  # pylint: disable=E1102
